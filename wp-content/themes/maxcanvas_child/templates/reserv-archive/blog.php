		...
		<div class="container-xl">
			<div class="row">
				<?php foreach( $all_posts as $post_i ):?>
					<div class="col-6 mb-3 article" data-postId="<?=$post_i->ID;?>">
						<a href="<?=$post_i->guid;?>">
							<div class="blog-archive--article-bg" style="background-image:url(<?=wp_get_attachment_url( get_post_thumbnail_id($post_i->ID) );?>);"></div>
						</a>
						<div class="blog-date-gradient text-uppercase mt-2 mb-2"><?=date_format( date_create($post_i->post_date),"d M Y" );?></div>
						<div class="blog-archive-sub-banner--title text-capitalize mb-3">
							<a href="<?=$post_i->guid;?>"><?=$post_i->post_title;?></a>
						</div>
					</div>
				<?php endforeach;?>
			</div>

			<section id="blog_pagination" class="blog-pagination">
				<div class="container">
					<!--pagination-->
